      ...
      <div className="container">
        ...
        <div className={`glass-card ${styles.reminderBar}`}>
          <div className={styles.reminderContent}>
            <h3>🔔 Never Miss a Live Build Session</h3>
          ...
          {CLASSES_DATA.map((c, i) => (
            <div key={i} className={`glass-card ${styles.card} ${c.status === 'LIVE NOW' ? styles.liveCard : ''}`}>
              <div className={styles.cardHeader}>
                ...
                <span className={styles.dateTime}>📅 {c.date} | ⏱️ {c.time}</span>
              </div>
              ...
              <div className={styles.cardFooter}>
                ...
                {c.status === 'UPCOMING' && (
                  <button className="btn btn-outline-gold w-full" style={{ justifyContent: 'center' }} disabled>
                    Streaming Starts in {c.date} ⏳
                  </button>
                )}
                {c.status === 'COMPLETED' && (
